...
机械革命无界14x 笔记本电脑配备了一款 **裕太微 YT6801 千兆以太网卡**。该网卡在 Linux 下需要单独安装驱动。官方提供了驱动源码包（版本 1.0.31），但在较新的内核（如 Linux 6.17）上编译时会因内核 API 变更而失败。本文记录了在 **AnduinOS**（基于 Ubuntu 22.04/24.04，内核 6.17.0-12-generic）上成功安装并启用 YT6801 驱动的完整过程。

## 环境信息

- 笔记本：机械革命无界14x  
- 操作系统：AnduinOS（Ubuntu 软件包基础，内核版本 6.17.0-12-generic）  
- 网卡：YT6801（裕太微电子）  
- 目标：使有线网卡正常工作（千兆速率）
...
但脚本执行失败，提示 `Fail to make`。手动进入源码目录执行 `make` 出现编译错误：

```
fuxi-gmac-common.c:75:23: error: ‘FXGMAC_INT_MODERATION_ENABLED’ undeclared
```
